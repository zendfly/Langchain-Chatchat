...
from chatchat.utils import build_logger, Sample
...
async def chat_completions(
    ...
    # 内容安全检查
    latest_content = {"content":body.messages[-1]['content']}
    security_check_result = Sample.content_security(f"{latest_content}")
    if isinstance(security_check_result, Exception):
        pass
    else:
        if security_check_result["advice_answer"] is not None:
            ...
            ret = OpenAIChatOutput(
                ...
                model=body.model,
                content=security_check_result['advice_answer'],
                finish_reason="stop",
    ...
    if body.tools:
        ...
        # img2txt
        # _upload_image = extra.get("upload_image")       # webui 接口
        _upload_image = extra.get("image_path")         # 自定义api接口
        if _upload_image != None:
            ...
            b64_img = encode_image("/usr/local/lib/python3.11/site-packages/chatchat/data/20230316113739941_ordinary.jpg")
            _latest_message = body.messages[-1]['content']
            body.messages[-1]['content'] = [{'type':"text", "text":_latest_message},
                                            {"type": "image_url", 
                                             "image_url":{"url":f"data:image/jpeg;base64,{b64_img}"}}]

            if body.model != Settings.model_settings.DEFAULT_VL_LLM_MODEL:
                body.model = Settings.model_settings.DEFAULT_VL_LLM_MODEL
            # rich.print(body)
